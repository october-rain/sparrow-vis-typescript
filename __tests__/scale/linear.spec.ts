...
function nice(domain: [number, number], niceDomain: [number, number], tickCount: number) {
...
function ticks(domain: [number, number], tickCount: number) {
...
describe("createLinear", () => {
  ...
  test("createLinear(options) uses custom interpolate.", () => {
    const s = createLinear({
      domain: [0, 1],
      range: ["a", "z"],
      interpolate: (t: number, start: string, end: string) => {
